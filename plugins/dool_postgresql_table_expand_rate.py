...
class dstat_plugin(dstat):
    """
    Plugin for PostgreSQL expand rate.
    """

    def __init__(self):
        self.name = 'postgresql table_expand_rate'
    ...
    def sort_result(self, key, result):
        ...
        result_ = self.remove_relpages(result_)
        result_ = self.completion_result(result_)
        return result_

    def remove_relpages(self, result):
        for i in range(len(result), 0, -1):
            if result[i-1][16] < 1000:
    ...
    def extract(self, outputfile):
        try:
            ...
            if self.last_out == []:
                ...
                expand_rate_ = self.sort_result('expand_rate', result)

                line1 = '-----------------------------------------------------------------------------postgresql-table_expand_rate----------------------------------------------------------------------------'
                line2 = ''
                for i in self.vars:
                    line2 += i + '  '
