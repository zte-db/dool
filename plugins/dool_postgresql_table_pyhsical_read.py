...
class dstat_plugin(dstat):
    """
    Plugin for PostgreSQL pyhsical_read.
    """

    def __init__(self):
        self.name = 'postgresql table_pyhsical_read'
    ...
    def extract(self, outputfile):
        try:
            ...
            if self.last_out == []:
                ...
                heap_blks_read_ = self.sort_result('heap_blks_read', result)

                line1 = '-----------------------------------------------------------------------------postgresql-table_physical_read----------------------------------------------------------------------------'
                line2 = ''
                for i in self.vars:
                    line2 += i + '  '
