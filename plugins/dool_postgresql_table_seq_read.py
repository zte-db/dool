...
class dstat_plugin(dstat):
    """
    Plugin for PostgreSQL seq_read.
    """

    def __init__(self):
        self.name = 'postgresql table_seq_read'
    ...
    def extract(self, outputfile):
        try:
            ...
            if self.last_out == []:
                ...
                seq_tup_read_ = self.sort_result('seq_tup_read', result)

                line1 = '-----------------------------------------------------------------------------postgresql-table_seq_read----------------------------------------------------------------------------'
                line2 = ''
                for i in self.vars:
                    line2 += i + '  '
